...
const App: React.FC = () => {
  ...
  // Timer logic for Timed Exam mode
  useEffect(() => {
    let timer: any;
    if (session && session.mode === QuizMode.TIMED && session.timeRemaining > 0 && !session.endTime) {
      timer = setInterval(() => {
        setSession(prev => {
          if (!prev) return null;
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#020617] text-white p-6 relative overflow-hidden">
      ...
      <div className="z-10 text-center max-w-3xl">
        ...
        <h1 className="text-6xl md:text-8xl font-black mb-6 tracking-tighter leading-[0.9] text-transparent bg-clip-text bg-gradient-to-b from-white to-slate-500">
          NCAA COMMAND <br/><span className="text-yellow-400">PRO PLATFORM</span>
        </h1>
        <p className="text-xl text-slate-400 mb-12 font-medium max-w-xl mx-auto leading-relaxed">
          The ultimate certification platform for aviation professionals. Strategic readiness for Pilots, Engineers, and ATC.
        </p>
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group">
          ...
          <div className="flex items-end gap-2"><span className="text-5xl font-black text-slate-900 tracking-tighter">{questions.length}</span><span className="text-slate-400 font-bold text-xs mb-1.5">Live DB</span></div>
        </div>
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {stats.history.length > 0 ? stats.history.slice(0, 3).map((h, i) => (
              <div key={i} className="flex items-center justify-between p-6 bg-white rounded-2xl border border-slate-100">
                <div className="flex items-center gap-4">
                  ...
                  <div><p className="font-black text-slate-900 text-sm uppercase">{h.trade}</p><p className="text-xs text-slate-400">{new Date(h.date).toLocaleDateString()}</p></div>
                </div>
              </div>
        ...
        <section className="bg-slate-950 rounded-3xl p-8 text-white relative overflow-hidden flex flex-col justify-between group">
          ...
          <button onClick={() => { startQuiz(Trade.SAFETY_HUMAN_FACTORS, QuizMode.TIMED); }} className="px-10 py-4 bg-white text-slate-950 font-black rounded-xl uppercase hover:bg-yellow-400 transition-all">Scramble Now</button>
        </section>
      </div>
    </div>
  );
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 pb-32">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => { if(confirm("Abort mission?")) setView('DASHBOARD'); }} className="px-4 py-2 bg-slate-100 rounded-xl text-[10px] font-black text-slate-900 uppercase">✕ Abort</button>
            <span className="text-[10px] font-black text-slate-400 uppercase">Sector: <span className="text-blue-600">{session.trade}</span></span>
          </div>
        ...
        <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-xl border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {q.options.map(opt => {
              ...
              return (
                <button key={opt.id} disabled={isAnswered} onClick={() => handleAnswer(q.id, opt.id)} className={`w-full p-6 rounded-2xl border-2 text-left transition-all flex items-center gap-5 ${styles}`}>
                  <div className={`w-10 h-10 rounded-xl border-2 flex items-center justify-center font-black text-sm ${isSelected ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-200 text-slate-400'}`}>{opt.id}</div>
          ...
          {isAnswered && (
            <div className={`mt-10 p-6 rounded-2xl border-2 ${userChoice === q.correctAnswer ? 'bg-green-50 border-green-100 text-green-800' : 'bg-red-50 border-red-100 text-red-800'}`}>
              <p className="font-black uppercase text-xs mb-2">{userChoice === q.correctAnswer ? 'Verified' : 'Error Detected'}</p>
        ...
        <div className="flex justify-end">
          <button onClick={nextQuestion} disabled={!isAnswered} className={`px-10 py-4 bg-slate-950 text-white rounded-xl font-black uppercase tracking-widest transition-all ${!isAnswered ? 'opacity-30' : 'hover:bg-blue-600'}`}>
    ...
    return (
      <div className="space-y-10">
        <header className="flex justify-between items-center">
          ...
          <button onClick={() => setShowBulk(!showBulk)} className={`px-6 py-3 rounded-xl font-black uppercase text-xs ${showBulk ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'}`}>
        ...
        {showBulk ? (
          <div className="bg-slate-900 rounded-[2rem] p-8 text-white">
            ...
            <textarea className="w-full h-64 bg-slate-950 rounded-xl border border-slate-800 p-6 font-mono text-sm mb-6" placeholder="Paste data samples..." value={bulkText} onChange={(e) => setBulkText(e.target.value)} />
            <div className="flex gap-4 mb-10">
              <select className="bg-slate-800 p-4 rounded-xl font-bold" value={bulkTrade} onChange={(e) => setBulkTrade(e.target.value as Trade)}>
                {Object.values(Trade).map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <button onClick={handleBulkProcess} disabled={isParsing || !bulkText} className="bg-blue-600 px-8 rounded-xl font-black uppercase">{isParsing ? 'Parsing...' : 'Analyze'}</button>
            </div>
            {parsedPreview.length > 0 && (
              <div className="space-y-4">
                <p className="text-green-400 font-black text-xs uppercase">Prepared: {parsedPreview.length} Modules</p>
                ...
                <button onClick={handleBatchSync} disabled={isSyncing} className="w-full py-5 bg-white text-slate-900 font-black rounded-xl uppercase">Initiate Sync</button>
              </div>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <section className="lg:col-span-2 bg-white rounded-[2rem] border shadow-sm overflow-hidden">
               <div className="p-6 border-b bg-slate-50 font-black uppercase text-sm">Registry</div>
               <div className="overflow-x-auto">
                 <table className="w-full text-left">
                   <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase"><tr><th className="px-6 py-4">Question</th><th className="px-6 py-4">Sector</th><th className="px-6 py-4">Action</th></tr></thead>
                   <tbody className="divide-y">
                     {questions.slice(0, 15).map(q => (
                       <tr key={q.id} className="hover:bg-slate-50">
                         ...
                         <td className="px-6 py-4"><button onClick={() => { if(confirm("Purge?")) apiService.deleteQuestion(q.id).then(() => setQuestions(prev => prev.filter(i => i.id !== q.id))); }} className="text-red-500 font-black text-xs uppercase">Purge</button></td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
  ...
  const ProfileView = () => (
    <div className="max-w-2xl mx-auto py-10 text-center space-y-10">
      <div className="w-32 h-32 rounded-full border-4 border-blue-600 mx-auto p-1 flex items-center justify-center text-4xl bg-slate-100 font-black text-slate-400">{user?.name?.[0] || 'A'}</div>
      <h2 className="text-3xl font-black uppercase text-slate-900">{user?.name || 'Asset'}</h2>
      <div className="grid grid-cols-2 gap-6 text-center">
        <div className="bg-white p-6 rounded-2xl border shadow-sm"><p className="text-[10px] font-black text-blue-400 uppercase">Avg Readiness</p><p className="text-3xl font-black">{stats.averageScore}%</p></div>
        <div className="bg-white p-6 rounded-2xl border shadow-sm"><p className="text-[10px] font-black text-indigo-400 uppercase">Clearance</p><p className="text-xl font-black text-indigo-600 uppercase">{user?.role}</p></div>
      </div>
      <button onClick={() => { apiService.clearAuth(); window.location.reload(); }} className="px-10 py-4 bg-red-600 text-white rounded-xl font-black uppercase text-xs">Revoke Authorization</button>
    </div>
  );
  ...
  return (
    <Layout currentView={view} setView={setView} user={user}>
      ...
      {view === 'TRADE_SELECT' && (
        <div className="space-y-10">
          <header><h2 className="text-4xl font-black uppercase">Deployment Sectors</h2></header>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TRADE_INFO.map(info => (
              <div key={info.trade} onClick={() => setCurrentTrade(info.trade)} className={`cursor-pointer p-8 rounded-[2rem] border-2 transition-all ${currentTrade === info.trade ? 'border-blue-600 bg-blue-50 shadow-lg' : 'border-slate-100 bg-white hover:border-slate-200 shadow-sm'}`}>
                <div className={`w-14 h-14 ${info.color} rounded-xl flex items-center justify-center text-3xl mb-6 shadow-md`}>{info.icon}</div>
                <h3 className="font-black text-xl uppercase text-slate-900">{info.trade}</h3>
                <p className="text-xs font-bold text-slate-400 mt-2 uppercase">{questions.filter(q => q.trade === info.trade).length} Modules Ready</p>
              </div>
            ))}
          </div>
          {currentTrade && (
            <div className="fixed inset-x-0 bottom-0 p-8 bg-white/95 backdrop-blur-xl border-t z-50">
              <div className="max-w-4xl mx-auto flex items-center justify-between">
                <div><h3 className="text-xl font-black uppercase">{currentTrade}</h3></div>
                <div className="flex gap-4">
                  <button onClick={() => startQuiz(currentTrade, QuizMode.PRACTICE)} className="px-8 py-4 bg-slate-100 font-black rounded-xl uppercase">Practice</button>
                  <button onClick={() => startQuiz(currentTrade, QuizMode.TIMED)} className="px-8 py-4 bg-blue-600 text-white font-black rounded-xl uppercase shadow-lg">Final Exam</button>
                </div>
              </div>
            </div>
          )}
      ...
      {view === 'RESULTS' && (
        <div className="max-w-xl mx-auto py-20 text-center space-y-8">
           <div className="bg-white p-12 rounded-[2rem] border shadow-xl">
            <h2 className="text-4xl font-black uppercase mb-4">Mission Success</h2>
            <p className="text-slate-500 font-medium mb-10">Operation data has been synchronized.</p>
            <div className="flex gap-4">
              <button onClick={() => setView('DASHBOARD')} className="flex-1 px-8 py-5 bg-slate-100 rounded-xl font-black uppercase">Home</button>
              <button onClick={() => setView('REVIEW')} className="flex-1 px-8 py-5 bg-slate-950 text-white rounded-xl font-black uppercase shadow-lg">Review</button>
            </div>
           </div>
        </div>
      )}
      ...
      {view === 'REVIEW' && (
        <div className="space-y-10 pb-32 max-w-4xl mx-auto">
          <header className="flex justify-between items-center">
            ...
            <button onClick={() => setView('DASHBOARD')} className="px-6 py-3 bg-slate-900 text-white rounded-xl font-black text-xs uppercase">Close</button>
          </header>
            ...
            {session?.questions.map((q, idx) => {
              ...
              return (
                <div key={q.id} className={`p-8 rounded-[2rem] border-2 bg-white ${isCorrect ? 'border-green-100' : 'border-red-100'}`}>
                  <div className="flex items-start gap-4 mb-6"><div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>{idx + 1}</div><h4 className="text-xl font-bold pt-1">{q.text}</h4></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                    {q.options.map(opt => (<div key={opt.id} className={`p-4 rounded-xl border text-sm font-bold flex items-center gap-3 ${opt.id === q.correctAnswer ? 'bg-green-50 border-green-200 text-green-900' : opt.id === userAns ? 'bg-red-50 border-red-200 text-red-900' : 'bg-slate-50 border-transparent text-slate-400 opacity-60'}`}><span className="w-6 h-6 rounded flex items-center justify-center border border-current">{opt.id}</span> {opt.text}</div>))}
                  </div>
                  <div className="p-6 rounded-2xl bg-slate-900 text-white text-sm font-medium leading-relaxed opacity-90">{q.explanation}</div>
                </div>
              );
            })}
          </div>
      ...
      {view === 'FLEET_SPECS' && <div className="p-10"><h2 className="text-4xl font-black uppercase mb-10">Fleet Intelligence</h2><button onClick={() => setView('LANDING')} className="text-blue-600 font-black uppercase">← Back</button></div>}
    </Layout>
  );
};
